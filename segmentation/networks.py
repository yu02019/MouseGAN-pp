"""
revised from https://github.com/HsinYingLee/MDMM
"""
import numpy as np
...
'''
'''
class enc_c_gen(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(enc_c_gen, self).__init__()
        enc_c = []
        tch = 64
        enc_c += [LeakyReLUConv2d(input_dim, tch, kernel_size=7, stride=1, padding=3)]
        for i in range(1, 3):
        ...
        self.conv_for_concat1 = LeakyReLUConv2d(256*2, 256, kernel_size=3, stride=1, padding=1)
        self.conv_for_concat2 = LeakyReLUConv2d(128*2, 128, kernel_size=3, stride=1, padding=1)
        self.conv_for_concat3 = LeakyReLUConv2d(64*2, 64, kernel_size=3, stride=1, padding=1)

    def forward(self, x):
        e0 = self.conv[0](x)
        e1 = self.conv[1](e0)  # 64 -> 128
        e2 = self.conv[2](e1)  # 128 -> 256
        e3 = self.conv[3](e2)  # 256 -> 256
        e4 = self.conv[4](e3)  # 256 -> 256
        e5 = self.conv[5](e4)  # 256 -> 256
        e6 = self.conv[6](e5)  # 256 -> 256
        gaussian7 = self.conv[7](e6)
        d0 = self.dec[0](gaussian7)  # 256 -> 256
        d1 = self.dec[1](d0)  # 256 -> 256
        d2 = self.dec[2](d1)  # 256 -> 256

        d2 = torch.cat((e2, d2), dim=1)  # 256 -> 512
        d2 = self.conv_for_concat1(d2)  # 512 -> 256
        d_convtrans3 = self.dec[3](d2)  # 256 -> 128

        d_convtrans3 = torch.cat((e1, d_convtrans3), dim=1)
        d_convtrans3 = self.conv_for_concat2(d_convtrans3)
        d_convtrans4 = self.dec[4](d_convtrans3)  # 128 -> 64

        d_convtrans4 = torch.cat((e0, d_convtrans4), dim=1)
        d_convtrans4 = self.conv_for_concat3(d_convtrans4)
        d_convtrans5 = self.dec[5](d_convtrans4)  # 64 -> 1
